            ...
            <tr>
              ...
              <td class="left"><select name="carousel_module[<?php echo $module_row; ?>][position]">
				<?php if ($module['position'] == 'footer_new') { ?>
					<option value="footer_new" selected="selected">Только для карусели</option>
				<?php } else { ?>
					<option value="footer_new">Только для карусели</option>
				<?php } ?>
...
function addModule() {	
	...
	html += '      <option value="footer_new">Только для карусели</option>';
	html += '    </select></td>';
