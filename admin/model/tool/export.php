...
class ModelToolExport extends Model {
	...
	function storeManufacturersIntoDatabase( &$database, &$products, &$manufacturerIds ) {
		...
		//SELECT manufacturer_id FROM manufacturer_to_store
		$manufacturer_bd = array();
		$sql3 = "SELECT `manufacturer_id` FROM `".DB_PREFIX."manufacturer_to_store`;";
		$result = $database->query( $sql3 );
		if ($result->rows) {
			foreach ($result->rows as $row) {
				$manufacturer_bd[] = $row['manufacturer_id'];
			}
		}
		//print "<pre>";
		//print_r($manufacturer_bd);
		//print "</pre>";
		...
		foreach ($products as $product) {
			...
			//поиск ID производителя в базе
			$search_manufacturerId_db = array_search($manufacturerId, $manufacturer_bd);
			//print "Найдено: ".$search_manufacturerId_db."<br>";
			...
			foreach ($storeIds as $storeId) {
				if (!in_array($storeId,$storeIdsForManufacturers[$manufacturerId])) {
					...
					if (array_search($manufacturerId, $manufacturer_bd) == FALSE and array_search($manufacturerId, $manufacturer_bd) != 0) {
						//print "Добавляем данные в базу: ".array_search($manufacturerId, $manufacturer_bd)."<br>";
	...
	function uploadProducts( &$reader, &$database, $startRow) {
		...
		$file = DIR_LOGS.'log_i.txt';
		$current = file_get_contents($file);
		$current.=date("H:i:s")." k = ".$k."\n";
		
		for ($i = $startRow; $i < $k; $i+=1) {
			$j = 1;
			/*
			if ($isFirstRow) {
				$isFirstRow = FALSE;
			...
			*/
			...
			$current.=date("H:i:s")." i = ".$i.", j = ".$j." | "."productId = ".$productId."\n";
			if ($productId=="") {
				continue;
			}
			...
			// если в названии изображения встречается http://, необходимо сначала загрузить файл на наш сервер. 
			// загружаем с помощью функции get_image
			//$file = DIR_LOGS.'log_get_image.txt';
			//$current = file_get_contents($file);
			//$current.="ID: ".$productId." IMAGE:".$imageName."\n";
			if (strpos($imageName, "http") !== false) {
				 $imageName = $this->get_image($imageName, $productId);
			}
			//$current.="ID: ".$productId." CHANGEIMAGE:".$imageName."\n";			
			//$current.="ID: ".$productId." STRPOS:".strpos($imageName, "http")."\n";
			
			
			$product['image'] = $imageName;			
			$product['shipping'] = $shipping;
			//$product['price'] = preg_replace("/[^0-9]/", '', $price);
			$product['price'] = $price;
	...
	function uploadCategories( &$reader, &$database, $StartRow) 
	{
	...
	function storeOptionsIntoDatabase( &$database, &$options ) 
	{
		...
		//открываем логи для записи
		//$file = DIR_LOGS.'log_options.txt';
		//$current = file_get_contents($file);
		...
		//вычисляем ID для product_option
		$sql = "SELECT `product_option_id` FROM `".DB_PREFIX."product_option`;\n";
		$result = $database->query( $sql );
		$maxProductOptionId = $result->num_rows;

		//$current.=$result->num_rows." \n";

		//file_put_contents($file, $current);	
		
		//вычисляем ID для OptionValue
		$sql = "SELECT `product_option_value_id` FROM `".DB_PREFIX."product_option_value`;\n";
		$result = $database->query( $sql );
		$maxProductOptionValueId = $result->num_rows;
	...
	function uploadOptions( &$reader, &$database, $startRow ) 
	{
		...
		*/	
			$productId = trim($this->getCell($data,$i,$j++));
			//$current.=date("H:i:s")." StartRow: ".$startRow."; i = ".$i."; j = ".$j."; i = ".$i."; k = ".$k."; productId = ".$productId." \n";
			if ($productId=="") {
				continue;
			}
	...
	function uploadAttributes( &$reader, &$database, $startRow) 
	{
		// find the default language id
		$languageId = $this->getDefaultLanguageId($database);
	...
	*/
	...
	function storeSpecialsIntoDatabase( &$database, &$specials )
	{
		...
		$sql = "SELECT `product_special_id` FROM `".DB_PREFIX."product_special`;\n";
		$result = $database->query( $sql );
		$productSpecialId =  $result->num_rows;
		//$productSpecialId = 0;
	...
	function uploadSpecials( &$reader, &$database, $startRow) 
	{
	...
	*/		
	...
	function storeDiscountsIntoDatabase( &$database, &$discounts )
	{
		...
		$sql = "SELECT `product_discount_id` FROM `".DB_PREFIX."product_discount`;\n";
		$result = $database->query( $sql );
		$productDiscountId =  $result->num_rows;
		//$productDiscountId = 0;
	...
	function uploadDiscounts( &$reader, &$database, $startRow) 
	{
	...
	*/
	...
	function storeRewardsIntoDatabase( &$database, &$rewards )
	{
		...
		if (! isset($_SESSION['productRewardId'])) {
			$productRewardId = 0;
		} else { 
			$productRewardId = $_SESSION['productRewardId'];
		}
		...
		$database->query("COMMIT;");
		$_SESSION['productRewardId'] = $productRewardId;
		return TRUE;
	}


	function uploadRewards( &$reader, &$database, $startRow ) 
	{
	...
	*/
	...
	function storeAdditionalImagesIntoDatabase( &$database, &$images )
	{
		...
		$sql = "SELECT `product_image_id` FROM `".DB_PREFIX."product_image`;\n";
		$result = $database->query( $sql );
		$productImageId =  $result->num_rows;
	...
	function uploadAdditionalImages( &$reader, &$database, $startRow) 
	{
	...
	*/
			...
			if (strpos($image, "http://") != false) {
				$image = get_image($image, $productId);
			}
	...
	function SearchEndFile( $objPHPExcel, $startRow, $chunkSize) {
		// ЛОГИ
		// открываем логи для записи
		//$file = DIR_LOGS.'log_memory_usage.txt';
		//$current = file_get_contents($file);
		//$current.= date("H:i:s")."	\n";
		...
		//$exit = false;
		//ищем конец файла в списке product (потом надо написать более продвинутый вариант)
		for ($k = 1; $k < 7; $k++) {
			//$current.= "k = ".$k."; exit = ".$exit.";\n";
			...
			for ($i = $startRow; $i < $startRow + $chunkSize; $i++) 	//внутренний цикл по строкам
			{
				...
				{	
					...
					$exit[$k] = 1;	
					//$current.= "k = ".$k."; exit = true; i = ".$i." \n";				
					//$current.= "Найдено более трех пустных строк. Выход... <br>";
								//continue;		
				}
			...
			//$current.= "k = ".$k."; exit = ".$exit[$k]."; empty_value = ".$empty_value."; i = ".$i." \n";
			//if ($empty_value == 0) 
			//	$exit = false;
			unset($objWorksheet);
	...
	//Создание каталогов и загрузка файла по URL
	function get_image($url_image, $product_id) {
		// Желаемая структура папок
		$structure = DIR_IMAGE."data/download/";
		$structure_4db = "data/download/";
		$path = $structure.$product_id.".jpeg";
		...
		$result = $structure_4db.$product_id.".jpeg";;

		$file = DIR_LOGS.'log2_get_image.txt';
		$current = file_get_contents($file);
		$current.="GET_IMAGE: ".$url_image."\n";
		//$current.=$status;
		file_put_contents($file, $current);
		return $result;
	}
	...
	*/			
		/*	some vars	*/
		$chunkSize = 101;		//размер считываемых строк за раз
		...
		echo "Импортировано: ".$startRow." строк";
		...
		//считывать только данные, без форматирования (экономит память)
		...
		if (! isset($_SESSION['isFirstEnter'])) {
			$isFirstEnter = TRUE;
		} else { 
			$isFirstEnter = $_SESSION['isFirstEnter'];
		}
		...
		// $file = DIR_LOGS.'log_memory_usage.txt';
		// $current = file_get_contents($file);
		// $current.=date("H:i:s")." Сколько используется памяти (перед циклом): ".(memory_get_usage()/1024/1024)."Mb \n";
		// $current.=date("H:i:s")." Кол-во строк: ".$startRow." \n";	
		...
		//ищем конец файла
		$exit = $this->SearchEndFile( $objPHPExcel, $startRow, $chunkSize);
		/*		
		$ok = $this->validateUpload( $objPHPExcel );
		if (!$ok) {
			echo "Ошибка!";
			return FALSE;
			break;
		...
		*/
		...
		if ($startRow == 1) { $starti = 1;} 
		else {$starti = $startRow - 1;}
				
		$ok = $this->uploadCategories( $objPHPExcel, $database, $starti);
		if (!$ok) {
			//echo "Ошибка! С категориями не удалось!";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! С категориями не удалось!')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		//echo "Категории импортировались!";
		
		$ok = $this->uploadProducts( $objPHPExcel, $database, $starti);
		if (!$ok) {
			echo "Ошибка! Проблема с товарами!";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! Проблема с товарами!')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		//echo "Товары импортировались!";
		
		//загрузка изображений 
				
		$ok = $this->uploadAdditionalImages( $objPHPExcel, $database, $starti);
		if (!$ok) {
			echo "Ошибка! Проблема с дополнительными изображениями!";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! Проблема с дополнительными изображениями!')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		$ok = $this->uploadOptions( $objPHPExcel, $database, $starti);
		if (!$ok) {
			echo "Ошибка! Проблема с опциями";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! Проблема с опциями')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		$ok = $this->uploadAttributes( $objPHPExcel, $database, $starti);
		if (!$ok) {
			echo "Ошибка! Атрибуты";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! Атрибуты')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		$ok = $this->uploadSpecials( $objPHPExcel, $database, $starti);
		if (!$ok) {
			echo "Ошибка! Акции";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! Акции')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		$ok = $this->uploadDiscounts( $objPHPExcel, $database, $starti);
		if (!$ok) {
			echo "Ошибка! Скидки";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! Скидки')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		$ok = $this->uploadRewards( $objPHPExcel, $database, $starti);
		if (!$ok) {
			echo "Ошибка! Отзывы";
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('Ошибка! Отзывы')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
			break;
		}
		...
		$isFirstEnter = FALSE;
		$_SESSION['isFirstEnter'] = $isFirstEnter;
			/*
			// Логи 
			//	$chunk++;
			//	$current.=date("H:i:s")." Сколько используется памяти ".$chunk." :".(memory_get_usage()/1024/1024)."Mb \n";
			//}
			...
			//$current.=date("H:i:s")." Кол-во шагов: ".$chunk." \n";
					
			$current.=date("H:i:s")." Сколько используется памяти: ".(memory_get_usage()/1024/1024)."Mb \n";
			$current.=date("H:i:s")." Пиковое использование памяти: ".(memory_get_peak_usage()/1024/1024)."Mb \n";
			...
			$current.= "Файл импортирован за: ".$result_time." сек. \n";
			...
			*/
			echo "<br /> Пиковое использование памяти: ".round(memory_get_peak_usage()/1024/1024)." Mb <br />";
			echo "Все хорошо, продолжаем импорт...";
		return $exit;
	...
	function populateProductsWorksheet( &$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, &$textFormat )
	{
		...
		//logs
		//$file = DIR_LOGS.'log_download.txt';
		//$current = file_get_contents($file);
